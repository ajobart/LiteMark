...
const Sidebar: React.FC<SidebarProps> = ({
    ...
    return (
        <div className="w- h-full overflow-scroll bg-background-sidebar p-4 flex flex-col">
            ...
            {/* SEARCH */}
            <div
                className='mt-2 mb-4 w-full bg-background-search p-2 box-border rounded-md border-2 border-background-border h-10 flex items-center justify-start gap-2'>
                                ...
                                getTagsWithCounts().filter(([tag]) =>
                                    ...
                                    <li key={tag} className="cursor-pointer">
                                        <div onClick={() => toggleTag(tag)}
                                             className="flex justify-between items-center p-2 rounded hover:bg-background-selected text-[#9CA3AF]">
                                            <span>{tag} ({count})</span>
                                            ...
                                            <ul className="flex flex-col gap-2">
                                                {notes.filter(note =>
                                                    ...
                                                    <li key={note.id}
                                                        ...
                                                        <div
                                                            className='h-full flex items-start justify-start flex-col overflow-hidden'>
                                                                ...
                                                                {note.tags && note.tags.length > 0 && (
                                                                    <ul className="flex flex-row w-auto gap-1 items-start overflow-hidden">
                                                                        {note.tags.map((tag, index) => (
                                                                            <li key={index}
                                                                                className="inline-flex items-center justify-center rounded-md bg-gray-400/10 hover:bg-gray-400/20 transition ease-in-out duration-300 py-0.5 px-1 text-xs font-medium text-gray-400 ring-1 ring-inset ring-gray-400/20">
                                                                                {tag}
                                                                            </li>
                                                                        ))}
                                                                    </ul>
            ...
            {activeTab === 'notes' && (
                ...
                <div>
                    ...
                    {sortedNotes.length === 0 ? (
                        <p className="text-center mt-4">{searchQuery ? "No results found" : "No notes created."}</p>
                    ) : (
                        <ul className='flex flex-col gap-2'>
